...
import { LockKeyhole, Pause, Play, RotateCcw, Loader2 } from 'lucide-react'
...
export default function ExercisePlayer({ sequence, routine }: Props) {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      if (accumulated >= .04) {
        ...
        setTime(value => Math.min(total, value + delta))
      }
  ...
  // Stop the clock at the end; replay starts from the opening pose.
  ...
  return <section className="exercise-player" aria-label="Guided exercise player">
    <div className="exercise-theater">
      ...
      <div className="theater-topline"><span><i /> MOVEMENT STUDIO</span><span><LockKeyhole size={11} /> GUIDED VIEW</span></div>
      <div className="theater-heading"><p>{routine ? `ROUTINE PREVIEW / ${String(index + 1).padStart(2, '0')} OF ${String(sequence.length).padStart(2, '0')}` : 'EXERCISE DEMONSTRATION'}</p><h2 data-testid="playing-exercise">{exercise.name}</h2></div>
      ...
      <span className="theater-footnote">ILLUSTRATIVE 3D MOTION · NOT MOTION CAPTURE</span>
    </div>
