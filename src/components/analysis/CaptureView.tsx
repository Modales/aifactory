...
import type { AnalysisFrame, CameraView } from '@/lib/analysisApi'
...
interface Props { config: CaptureConfig; recording: boolean; epoch: number; overlapStart: number; onFrame: (id: string, frame: AnalysisFrame, aspect: number) => void; onReady: (id: string, ready: boolean) => void; onEnded: (id: string) => void }

export default function CaptureView({ config, recording, epoch, overlapStart, onFrame, onReady, onEnded }: Props) {
  ...
  useEffect(() => {
    ...
    async function prepare() {
      try {
        ...
        if (cancelled) return
        setReady(true); onReady(config.id,true); setMessage('Ready · keep one athlete fully visible')
      } catch (e) { if (!cancelled) { setMessage((e as Error).message); onReady(config.id,false) } }
    }
    void prepare()
