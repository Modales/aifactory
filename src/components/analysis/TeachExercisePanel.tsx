...
import type { TaughtExercise } from '@/lib/analysisApi'

interface Props {
  muscles: { id: string; name: string }[]
  working: boolean
  onTeach: (name: string, muscles: string[]) => Promise<TaughtExercise>
  onCancel?: () => void
}

/** Turn the set just recorded into a new library entry: the server learns range, tempo and rep gates from these reps. */
export default function TeachExercisePanel({ muscles, working, onTeach, onCancel }: Props) {
  ...
  async function submit() {
    ...
    try { setResult(await onTeach(name.trim(), picked)) } catch (e) { setError((e as Error).message) } finally { setBusy(false) }
  }
  ...
  return (
    <section className="record-card pad teach-panel">
      <p className="record-section-title"><GraduationCap size={13} style={{ display: 'inline', verticalAlign: '-2px', marginRight: 5 }} />Teach this exercise</p>
