...
export default function AnalysisStudio() {
  ...
  return <SocialShell><main className="analysis-layout"><Link to="/dashboard" className="text-link"><ArrowLeft size={15}/>Back to your feed</Link><div className="analysis-heading"><div><span className="eyebrow">TRAIN WITH INTENTION</span><h1>Your movement.<br/><span>A clearer picture.</span></h1></div><span className="analysis-private"><ShieldCheck size={16}/>Video stays on your device</span></div>
    ...
    <div className="analysis-columns"><div className="analysis-capture-column">
      ...
      {stage==='review'&&<section className="social-card analysis-review"><span className="eyebrow">YOUR SET, IN PERSPECTIVE</span><h2>{saved?'Workout saved. Nice work.':'Let’s look at the evidence.'}</h2><p>{saved?'Head to your feed to share this session with your crew.':'Review the visible checks before saving. No score means there was not enough evidence—not that your form was bad.'}</p><div className="capture-actions"><button className="social-button" disabled={working||saving} onClick={reset}>Analyze another set</button>{saved?<Link className="social-button primary" to="/dashboard"><Check size={16}/>Back to activity feed</Link>:<button className="social-button primary" disabled={working||saving||report?.score==null||!report.analysisId} onClick={save}><Save size={16}/>{saving?'Saving…':'Save workout'}</button>}</div></section>}
