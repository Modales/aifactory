...
export interface AnalysisReport { analysisId?: string; modelVersion: string; exercise: ExerciseId | null; exerciseName: string; family: string | null; familyName: string | null; primaryJoint: string | null; candidate: ExerciseId | null; confidence: number; candidates: (NamedExercise & { score: number })[]; alternatives: NamedExercise[]; selectionSource: string; status: string; repCount: number; score: number | null; durationSeconds: number; reps: AnalysisRep[]; focus: FocusArea[]; headline: string; views: {id: string; view: string; viewSource: string}[]; warnings: string[]; notAssessed: string[]; disclaimer: string }
...
export const teachExercise = (name: string, muscles: string[], streams: CameraStream[], synchronized: boolean) => request<TaughtExercise>('/api/analysis/exercises', { method: 'POST', body: { name, muscles, streams, synchronized } })
