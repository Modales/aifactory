...
- `/exercises` is the public guided Movement Library: 24 authored demonstrations across Strength foundations, Bodybuilding accessories, and Mobility & rehab, plus four routine outlines. `exerciseLibrary.ts` owns the catalog/prescriptions; `exerciseAnimation.ts` owns deterministic poses and explicit BodyParts3D joint weights; `exerciseCamera.ts` owns the three-shot automatic camera choreography. The lazy `ExerciseScene` uses a shared skinned skeleton and a curated 6.6 MB asset subset in `public/models/exercise-*`. Keep attribution updated when adapting the assets. These are illustrative motions, not captured biomechanics or measured muscle activation. Each routine previews two six-second reps per exercise, not its full training duration; play/pause/scrub and chapter buttons are local-only, and do not log a workout. Test with `npm test` (includes `tests/exerciseAnimation.test.ts`).
