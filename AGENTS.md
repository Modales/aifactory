...
- `/exercises` is the public guided Movement Library: six authored exercise demonstrations and three complete routine outlines. `exerciseLibrary.ts` owns the catalog/prescriptions; `exerciseAnimation.ts` owns deterministic poses and BodyParts3D joint weights. The lazy `ExerciseScene` uses a fixed camera (no orbit controls), a shared skinned skeleton, and a curated 6.6 MB asset subset in `public/models/exercise-*`. Keep attribution updated when adapting the assets. These are illustrative motions, not captured biomechanics or measured muscle activation. Each routine previews two six-second reps per exercise, not its full training duration; play/pause/scrub and chapter buttons are local-only, and do not log a workout. Test with `npm test` (includes `tests/exerciseAnimation.test.ts`).
